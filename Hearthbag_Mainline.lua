...
function hearthCleanup.BagsUpdated()
    print("debug combined bag running")
    if GetCVarBool("combinedBags") == true then
        print("debug combined bags true")
    elseif GetCVarBool("combinedBags") == false then
        print("debug combined bags false")
    else
    ...
    if GetCVarBool("combinedBags") == true and HearthDB.BAG["parent"] == "BagItemAutoSortButton" and HearthDB.BAG["position"][1] == -142 and HearthDB.BAG["position"][2] == 30 then
        ...
        hearthCleanup.tooltip = true
        print("debug was default, switch to combined")
    elseif GetCVarBool("combinedBags") == false and HearthDB.BAG["parent"] == "BagItemAutoSortButton" and HearthDB.BAG["position"][1] == -394 and HearthDB.BAG["position"][2] == 30 then
        ...
        hearthCleanup.UpdatePosition()
        hearthCleanup.tooltip = true
        print("debug was combined, switch to default")
    else
        hearthCleanup.tooltip = false
        print("debug everything else")
    end
end
    ...
    elseif HearthDB.BAG["parent"] == "BagItemAutoSortButton" then
        if hearthCleanup.tooltip == true then
            ...
            if ( hearthbag:GetID() == 0 ) then
                ...
                GameTooltip:AddLine(string.gsub(CLICK_BAG_SETTINGS, "Click", "Right-Click"));
            end
                local start, duration, enabled, modRate = GetSpellCooldown(8690);
                if duration ~= 0 then
                    --print("debug get time: " .. start+duration-GetTime() and date("%I:%M %p",time() + start+duration-GetTime()) )
                    GameTooltip:AddLine("Cooldown Remaining: " .. date("%H:%M:%S",time() + start+duration-GetTime()));
                end
            GameTooltip:Show();
        end
    end
